...
class Alphas101():
    ...
    def __init__(self, df:pd.DataFrame, alpha_model_name:str):
        self.alphaModel_name = 'alpha' +  alpha_model_name
        self.df_data = df.copy()
        self.open = self.df_data['Open']
        self.high = self.df_data['High']
        self.low = self.df_data['Low']
        self.close = self.df_data['Adj Close']
        self.volume = self.df_data['Volume']
        self.returns = (np.log(self.df_data['Open'] / self.df_data['Open'].shift())) #daliy close-to-close returns
        self.vwap = (self.volume * ((self.high + self.low + self.close) / 3)).cumsum() / self.volume.cumsum()
        # self.cap = self.df_data['Cap']
        alpha_method = getattr(self, self.alphaModel_name)  # Use self.alphaModel_name
        self.df_data['weights'] = alpha_method()
    ...
    def alpha021(self):
        cond_1 = sum(self.close, 8) + stddev(self.close, 8) < sum(self.close, 2)
        cond_2 = sum(self.volume, 20) / self.volume < 1
    ...
    def alpha023(self):
        cond = sum(self.high, 20) < self.high
    ...
    def alpha024(self):
        cond = delta(sum(self.close, 100), 100) / delay(self.close, 100) <= 0.05
    ...
    def alpha027(self):
        alpha = rank((sum(correlation(rank(self.volume), rank(self.vwap), 6), 2) / 2.0))
        result = pd.Series(1, index=self.volume.index)
        result[alpha > 0.5] = -1
    ...
    def alpha032(self):
        return scale(((sum(self.close, 7) / 7) - self.close)) + (
    ...
    def alpha056(self):
        return (0 - (1 * (rank((sum(self.returns, 10) / sum(sum(self.returns, 2), 3))) * rank((self.returns * self.cap)))))
    ...
    def alpha066(self):
        return ((rank(decay_linear(delta(self.vwap, 4).to_frame(), 7).CLOSE) + ts_rank(decay_linear(((((
            self.low * 0.96633) + (self.low * (1 - 0.96633))) - self.vwap) / (self.open - ((self.high + self.low) / 2))).to_frame(),
            11).CLOSE, 7)) * -1)
    ...
    def alpha071(self):
        ...
        df['max'] = df[['p1', 'p2']].max(axis=1)  # Use max along rows instead of at
        return df['max']
    ...
    def alpha077(self):
        ...
        df = pd.DataFrame({'p1': p1, 'p2': p2})
        df['min'] = df[['p1', 'p2']].min(axis=1)  # Use min along rows instead of at
        return df['min']
